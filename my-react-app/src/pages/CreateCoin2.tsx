...
const SniperProtectionPopup: React.FC<SniperProtectionPopupProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-900 border border-gray-600 rounded-lg p-6 max-w-md w-full mx-4 animate-slide-up">
                {/* Close Button */}
                <button
                    onClick={onClose}
...
function CreateCoin2() {
    ...
    // Add a new state to track the token creation process
    ...
    const handleSubmit = async () => {
        ...
        // Reset loading state after a short delay if popup is shown
        setTimeout(() => setIsCreatingToken(false), 500);
    };

    // New handler functions for sniper protection
const handleSniperProtectionAction = async (action: 'confirm' | 'skip', amount?: string) => {
    setIsCreatingToken(true);
    ...
    const createTokenWithProtection = async (protectionAmount: string) => {
        try {
            ...
            setIsCreatingToken(false); // Reset loading state
        }
    };
    ...
    return (
        <div className='w-full flex flex-col bg-custom-dark-blue'>
                        ...
                        <form className='flex flex-col justify-center w-full max-w-[500px] mx-auto mb-10 mt=10'>
                            ...
                            <div className='mb-6 p-4 bg-gradient-to-r from-purple-900/30 to-blue-900/30 rounded-lg border border-purple-500/30'>
                                ...
                                <ul className='text-sm text-gray-300 space-y-1'>
                                    ...
                                    <li>
                                        • Total Supply:{" "}
                                        {DEFAULT_CONFIG.totalSupply.toLocaleString()} tokens
                                    </li>
                            ...
                            <div className='flex justify-end mt-8'>
                                <button
                                    ...
                                    disabled={isCreatingToken || !connected || !publicKey}
                                >
                                    {isCreatingToken
                                        ? "Processing..."
                                        : !connected || !publicKey
            ...
            {showSniperPopup && (
                ...
                onConfirm={(amount) => handleSniperProtectionAction('confirm', amount)}
                onSkip={() => handleSniperProtectionAction('skip')}
